...
use crate::sys::c;
use crate::sys::windows::dur2timeout;
...
impl Condvar {
    ...
    pub unsafe fn init(&mut self) {
        *self.sleepWakeupSemaphore.get() =
            c::CreateSemaphoreW(ptr::null_mut(), 1, 1, ptr::null_mut());
        assert!(*self.sleepWakeupSemaphore.get() != ptr::null_mut());
        *self.wakeOneEvent.get() =
            c::CreateEventW(ptr::null_mut(), c::FALSE, c::FALSE, ptr::null_mut());
        assert!(*self.wakeOneEvent.get() != ptr::null_mut());
        *self.wakeAllEvent.get() =
            c::CreateEventW(ptr::null_mut(), c::TRUE, c::FALSE, ptr::null_mut());
    ...
    pub unsafe fn wait_timeout(&self, mutex: &Mutex, dur: Duration) -> bool {
        ...
        let waitResult =
            c::WaitForMultipleObjects(2, handeles.as_ptr(), c::FALSE, dur2timeout(dur));
        ...
        wcwm = self.sleepersCountAndWakeupMode.fetch_add(-sub as usize, Ordering::SeqCst)
            - sub as usize;
        ...
        } else if waitResult == c::WAIT_TIMEOUT
            ...
            && sleepersCount == 0
        {
            ...
            self.sleepersCountAndWakeupMode.store(WAKEUP_MODE_NONE, Ordering::SeqCst);
            releaseSleepWakeupSemaphore = true;
        } else if waitResult == c::WAIT_TIMEOUT && dur2timeout(dur) != c::INFINITE
